...
for(k in 1:length(tickers[,1]))
{
  tryit <- try(getQuote(paste(tickers[k,1]), what=what_metrics))
  if(inherits(tryit, "try-error")){
    k <- k+1
  } else {
    mc<-getQuote(paste(tickers[k,1]), what=what_metrics)
    tickers[k,9]<-mc[2]
  }
}
...
marketcap<-tickers[,c(1,4,9)]  
colnames(marketcap)<-c("Tickers","Sector","Market.Cap")
...
for(j in 1:11)
{
  ...
  for(i in 1:length(symbol))
  {
    ...
    if(inherits(tryit, "try-error")){
      ...
      getSymbols(symbol[i], from = "2009-01-01", src =  "yahoo", adjust =  TRUE)
